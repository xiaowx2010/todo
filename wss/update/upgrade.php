...
mysql_query("DELETE FROM `tankdb`.`tk_item` WHERE `tk_item`.`item_id`=2", $tankdb) or die(mysql_error());
mysql_query("DELETE FROM `tankdb`.`tk_item` WHERE `tk_item`.`item_id`=6", $tankdb) or die(mysql_error());
...
mysql_query("
INSERT INTO  `tankdb`.`tk_task_tpye` (
`task_tpye` ,
`task_tpye_backup1`
)
VALUES (
...
<div class="rowcon">
  ...
  <?php  if($update_rs == 0){ ?>  
  <p>&nbsp;</p>
  <p><span class="font_big18">WSS 1.3.0 升级说明</span><br/><br/>
  <span class="font_big18">注意，本升级脚本在Wamp环境的默认配置下，经过数十次测试，均可正常运行。由于不同的amp环境对数据库的操作权限不同，我们不保证在其他amp环境中可正常升级，如果您的环境中无法正常运行本升级脚本，我们建议您先将您正在使用的WSS中的数据库导出，然后使用一台windows环境的pc，参考 <a href="http://www.wssys.net/zh-CN/file.php?recordID=16&projectID=-1" target="_blank">5分钟安装说明</a> 部署一套Wamp环境的WSS1.2.9版并导入您刚导出的数据，在Wamp环境中运行升级脚本，对数据库进行升级后，再倒入至您其他amp环境中的WSS。</span><br/><br/>
  </p>
...
<p>数据库升级已经完成，请继续执行以下操作完成升级：</p>
	...
	<p>2) 使用 WSS 1.3.0 压缩包中的 WSS目录覆盖你服务器上的WSS目录（如修改过数据库连接文件config/tank_config.php，则升级后需要重新配置）；</p>
	<p>3) 删除服务器上本升级脚本文件（upgrade.php）；</p>
	<p>4) 重新登录后，可开始使用 WSS1.3.0，如升级后样式错乱，使用ctrl+F5 强制刷新；</p>
	<p>5) 1.2.8版本之前的用户升级后，可能出现用户不可见的情况，只要相应的用户登录一次系统即可。</p>
	<br /><br />

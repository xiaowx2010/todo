        ...
        if (h_actiondata.Length > 1)
        {
            ...
            if (h_actiondatacode.Length > 0)
            {
                ...
                if (ht.Count>0)
                {
                    ...
                    StringBuilder sbextmodule = new StringBuilder("0,");
                    
                    foreach (DictionaryEntry mcode in ht)
                    {
                        sbsql.Append("if exists (select 1 from gmis_moduleright where usergroupcode=" + id + " and modulecode=" + mcode.Key.ToString().Trim(';') + ") begin update gmis_moduleright set rightcontent='" + mcode.Value.ToString() + "' where usergroupcode=" + id + " and modulecode=" + mcode.Key.ToString() + " end else begin insert into gmis_moduleright (modulecode,usergroupcode,rightcontent) values (" + mcode.Key.ToString() + "," + id + ",'" + mcode.Value.ToString().Trim(';') + "') end;");
                        sbextmodule.Append(mcode.Key.ToString() + ",");
                       
                    }

                    if (sbsql.Length > 0)
                    {
                        sbsql.Append("delete gmis_moduleright where usergroupcode=" + id + " and modulecode not in (" + sbextmodule.ToString().Trim(',') + ");");
                        exeinfo = db.UpdateTable(db.ConnStr, sbsql.ToString());
                        Response.Redirect("edit_moduleright.aspx?&id=" + id);
...
private string GetBindActions(string usergroupcode)
{
    ...
    DataTable dt = db.GetDataTable(db.ConnStr, "select modulecode,rightcontent from gmis_moduleright where usergroupcode=" + id+" and rightcontent<>''");
    ...
    foreach (DataRow dr in dt.Rows)
    {
        ...
        {
            sbAction.Append(dr["modulecode"].ToString() + "-" + ac + ","); 
        }
    }
...
<tr>
    <td nowrap style="width:400px;">
        <div id="divModuleRight" style="width:400px; height:200px;">
        <script type="text/javascript">
        WriteFlash("MultiSelect7","images/MultiSelect.swf?app=MultiSelect7&mode=3&cols=4&width=400&height=200&dataroot=edit_moduleright_data.aspx&fold=1&sellist=<%=p_sellist%>", 400, 200, "");

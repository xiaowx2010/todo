        ...
        if (h_actiondata.Length > 1)
        {
            ...
            if (h_actiondatacode.Length > 0)
            {
                ...
                if (ht.Count>0)
                {
                    ...
                    StringBuilder sbextarea = new StringBuilder("0,");
                    
                    foreach (DictionaryEntry mcode in ht)
                    {
                        sbsql.Append("if exists (select 1 from gmis_arearight where usergroupcode=" + id + " and areacode=" + mcode.Key.ToString().Trim(';') + ") begin update gmis_arearight set rightcontent='" + mcode.Value.ToString() + "' where usergroupcode=" + id + " and areacode=" + mcode.Key.ToString() + " end else begin insert into gmis_arearight (areacode,usergroupcode,rightcontent) values (" + mcode.Key.ToString() + "," + id + ",'" + mcode.Value.ToString().Trim(';') + "') end;");
                        sbextarea.Append(mcode.Key.ToString() + ",");
                       
                    }

                    if (sbsql.Length > 0)
                    {
                        sbsql.Append("delete gmis_arearight where usergroupcode=" + id + " and areacode not in (" + sbextarea.ToString().Trim(',') + ");");
                        exeinfo = db.UpdateTable(db.ConnStr, sbsql.ToString());
                        Response.Redirect("edit_arearight.aspx?&id=" + id);
...
private string GetBindActions(string usergroupcode)
{
    ...
    DataTable dt = db.GetDataTable(db.ConnStr, "select areacode,rightcontent from gmis_arearight where usergroupcode=" + id+" and rightcontent<>''");
    ...
    foreach (DataRow dr in dt.Rows)
    {
        ...
        {
            sbAction.Append(dr["areacode"].ToString() + "-" + ac + ","); 
        }
    }
...
<tr>
    <td nowrap style="width:400px;">
        <div id="divareaRight" style="width:400px; height:200px;">
        <script type="text/javascript">
        WriteFlash("MultiSelect7","images/MultiSelect.swf?app=MultiSelect7&mode=3&cols=4&width=400&height=200&dataroot=edit_arearight_data.aspx&fold=1&sellist=<%=p_sellist%>", 400, 200, "");
